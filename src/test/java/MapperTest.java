...
public class MapperTest {
  ...
  public void should_convert_a_person_entity() {
    ...
    MapperFactory simpleMapperFactory = new DefaultMapperFactory.Builder().build();
    simpleMapperFactory.classMap(PersonEntity.class, PersonDto.class).byDefault()
        .field("password", "password")
        .field("password", "confirmpassword")
        .register();
    MapperFacade simpleMapperFacade = simpleMapperFactory.getMapperFacade();
    // When
    PersonDto pierreDto = simpleMapperFacade.map(pierre, PersonDto.class);
    // Then
  ...
  public void should_convert_a_person_entity_using_MasterMapperFactory() {
    ...
    MasterMapperFactory<PersonEntity, PersonDto> masterMapperFactory = new MasterMapperFactory(PersonEntity.class, PersonDto.class);
    masterMapperFactory.addField("password", "password");
    masterMapperFactory.addField("password", "confirmpassword");
    // When
    PersonDto pierreDto = masterMapperFactory.convertEntityToDto(pierreEntity);
    // Then
  ...
  public void should_convert_a_person_dto() {
    ...
    MapperFactory simpleMapperFactory = new DefaultMapperFactory.Builder().build();
    simpleMapperFactory.classMap(PersonEntity.class, PersonDto.class).byDefault().register();
    MapperFacade simpleMapperFacade = simpleMapperFactory.getMapperFacade();
    // When
    PersonEntity pierreEntity = simpleMapperFacade.map(pierreDto, PersonEntity.class);
    // Then
    isPersonEntityEqualsToPersonDto(pierreEntity, pierreDto);
  }
  ...
  public void should_convert_a_person_dto_using_MasterMapperFactory() {
    ...
    masterMapperFactory.addField("password", "confirmpassword");
    // When
    PersonEntity pierreEntity = masterMapperFactory.convertDtoToEntity(pierreDto);
    // Then
    isPersonEntityEqualsToPersonDto(pierreEntity, pierreDto);
  }

  @Test
  public void should_convert_a_list_of_person_entity_using_MasterMapperFactory() {
    // Given
    ...
    masterMapperFactory.addField("password", "confirmpassword");
    // When
    List<PersonDto> personDtos = masterMapperFactory.convertEntitiesToDtos(personEntities);
    // Then
    isPersonEntitiesEqualsToPersonDtos(personEntities, personDtos);
  }

  @Test
  public void should_convert_a_list_of_person_dto_using_MasterMapperFactory() {
    // Given
    ...
    masterMapperFactory.addField("password", "confirmpassword");
    // When
    List<PersonEntity> personEntities = masterMapperFactory.convertDtosToEntities(personDtos);
    // Then
    isPersonEntitiesEqualsToPersonDtos(personEntities, personDtos);
  }

  private void isPersonEntityEqualsToPersonDto(PersonEntity entity, PersonDto dto) {
    ...
    isRoleEntityEqualsToRoleDto(entity.getPrimaryRole(),dto.getPrimaryRole());
    isRoleEntitiesEqualsToRoleDtos(entity.getSecondaryRoles(),dto.getSecondaryRoles());
    if (entity.getCoworkers()!=null && dto.getCoworkers()!=null){
      isPersonEntitiesEqualsToPersonDtos(entity.getCoworkers(), dto.getCoworkers());
    }
  }

  private void isPersonEntitiesEqualsToPersonDtos(Set<PersonEntity> entities, Set<PersonDto> dtos) {
    ...
    while (entityIterator.hasNext()){
      isPersonEntityEqualsToPersonDto(entityIterator.next(), dtoIterator.next());
    }
  }


  private void isRoleEntityEqualsToRoleDto(RoleEntity roleEntity,RoleDto roleDto){
    assertThat(roleEntity.getName()).isEqualTo(roleDto.getName());
  }

  private void isRoleEntitiesEqualsToRoleDtos(List<RoleEntity> roleEntities,List<RoleDto> roleDtos){
  ...
  private void isPersonEntitiesEqualsToPersonDtos(List<PersonEntity> entities, List<PersonDto> dtos) {
    assertThat(entities).isNotNull();
    assertThat(dtos).isNotNull();
    assertThat(entities).hasSize(dtos.size());
  ...
  private PersonEntity createPierreEntity() {
    ...
    pierre.setSecondaryRoles(asList(secondaryRoleEntity1, secondaryRoleEntity2));
    return pierre;
  }
  ...
  private PersonEntity createFaycalEntity() {
    ...
    faycal.setSecondaryRoles(asList(secondaryRoleEntity1,secondaryRoleEntity2,secondaryRoleEntity3));
    return faycal;
  }
